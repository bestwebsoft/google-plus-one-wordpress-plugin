...
Plugin URI: https://bestwebsoft.com/products/wordpress/plugins/google-plus-one/
...
if ( ! function_exists( 'gglplsn_admin_menu' ) ) {
	function gglplsn_admin_menu() {
		...
		$settings = add_menu_page( __( 'Google +1 Settings', 'google-one' ), 'Google +1', 'manage_options', 'google-plus-one.php', 'gglplsn_options', 'dashicons-googleplus' );
		add_submenu_page( 'google-plus-one.php', __( 'Google +1 Settings', 'google-one' ), __( 'Settings', 'google-one' ), 'manage_options', 'google-plus-one.php', 'gglplsn_options' );

		add_submenu_page( 'google-plus-one.php', 'BWS Panel', 'BWS Panel', 'manage_options', 'gglplsn-bws-panel', 'bws_add_menu_render' );

		if ( isset( $submenu['google-plus-one.php'] ) )
			$submenu['google-plus-one.php'][] = array( 
				'<span style="color:#d86463"> ' . __( 'Upgrade to Pro', 'google-one' ) . '</span>',
				'manage_options',
				'https://bestwebsoft.com/products/wordpress/plugins/google-plus-one/?k=0a5a8a70ed3c34b95587de0604ca9517&pn=102&v=' . $gglplsn_plugin_info["Version"] . '&wp_v=' . $wp_version );
...
if ( ! function_exists ( 'gglplsn_init' ) ) {
	function gglplsn_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $gglplsn_plugin_info, '3.9' );/* check compatible with current WP version ##*/
...
if ( ! function_exists( 'gglplsn_options' ) ) {
	function gglplsn_options() {
		...
		$page = new Gglplsn_Settings_Tabs( plugin_basename( __FILE__ ) ); ?>
		<!-- general -->
...
if ( ! function_exists( 'gglplsn_action_links' ) ) {
	function gglplsn_action_links( $links, $file ) {
		if ( ! is_network_admin() ) {
			...
			if ( $file == $this_plugin && ! is_plugin_active( 'google-one-pro/google-plus-one-pro.php' ) && ! is_plugin_active( 'social-buttons-pack/social-buttons-pack.php' ) ) {
...
if ( ! function_exists( 'gglplsn_register_plugin_links' ) ) {
	function gglplsn_register_plugin_links( $links, $file ) {
		...
		if ( $file == $base ) {
			if ( ! is_network_admin() && ! is_plugin_active( 'google-one-pro/google-plus-one-pro.php' ) && ! is_plugin_active( 'social-buttons-pack/social-buttons-pack.php' ) )
				$links[]	=	'<a href="admin.php?page=google-plus-one.php">' . __( 'Settings', 'google-one' ) . '</a>';
			$links[]	=	'<a href="https://support.bestwebsoft.com/hc/en-us/sections/200538809" target="_blank">' . __( 'FAQ', 'google-one' ) . '</a>';
			$links[]	=	'<a href="https://support.bestwebsoft.com">' . __( 'Support', 'google-one' ) . '</a>';
		}
		return $links;
	}
}
